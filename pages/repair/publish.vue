<template>
  <view class="page">
    <view class="feedback-title">
      <text>问题、意见或者你想要的维修请求</text>
      <text class="feedback-quick" @tap="chooseMsg">快速键入</text>
    </view>
    <view class="feedback-body">
      <textarea
        placeholder="请详细描述需要维修的设施及所在位置..."
...
import { ApiPostRepair } from "@/api/repair.js";
export default {
  data() {
    return {
      msgContents: ["走廊灯坏", "门前卫生不干净", "电梯故障"],
  ...
  methods: {
    ...
    send() {
      ...
      wx.requestSubscribeMessage(
        {
          //订阅【报事报修完成通知】
          tmplIds:["KzKcSUKQeFSLfVTD-_7wRZLy8fk1uF1XYiWrObXSaDg"],
           success (res) { 
             console.log(res)
           }
        }
        ...
        .then((data) => {
          ...
          if (data.status === 0) {
            if (imgs.length > 0) {
              imgs.forEach((ele) => {
                uni.uploadFile({
                  url: that.$mStore.state.BaseUrl + "api/prorepair/prorepair/",
                  filePath: ele.uri,
                  name: ele.name,
                  header: {
                  ...
                  formData: { uuid: data.msg, method: "put", communityuuid:that.sendDate.communityuuid },
                  success: (res) => { 
